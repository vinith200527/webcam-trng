...
NIST_OUTPUT_FILE = "nist_data.txt"
...
async def generate_nist_file(total_bits: int):
    _init_db()
    _load_and_filter_camera_urls()
    bits_generated = 0
    if os.path.exists(NIST_OUTPUT_FILE):
        bits_generated = os.path.getsize(NIST_OUTPUT_FILE)
        logger.info(f"Found existing '{NIST_OUTPUT_FILE}' with {bits_generated} bits. Resuming.")
    if bits_generated >= total_bits:
        logger.info("Target number of bits already generated. Nothing to do.")
        return
    with open(NIST_OUTPUT_FILE, 'a') as f:
        while bits_generated < total_bits:
            if not _buffer: await _generate_and_store_numbers()
            while _buffer and bits_generated < total_bits:
                hex_val = await _pop_from_buffer_and_db()
                if not hex_val: break
                binary_str = hex_to_binary_string(hex_val)
                f.write(binary_str)
                f.flush()
                bits_generated += len(binary_str)
                progress = min(1.0, bits_generated / total_bits)
                print(f"\rProgress: [{'#' * int(progress * 40):<40}] {progress:.1%}", end="")
    print("\n")
    logger.info(f"Finished. Total bits in file: {bits_generated}.")

app = FastAPI(title="Webcam True RNG v3.9.6", version="3.9.6")
@app.on_event("startup")
async def startup_event():
    _init_db()
    _load_buffer_from_db()
    _load_and_filter_camera_urls()
    if len(_buffer) < BUFFER_SIZE: asyncio.create_task(_refill_buffer())

class RandomResponse(BaseModel): random_hex: str
@app.get("/random", response_model=RandomResponse)
async def get_random():
    if not _buffer: await _refill_buffer()
    value = await _pop_from_buffer_and_db()
    if not value: raise HTTPException(status_code=503, detail="Service unavailable.")
    if len(_buffer) < BUFFER_SIZE: asyncio.create_task(_refill_buffer())
    return RandomResponse(random_hex=value)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Webcam-based True Random Number Generator v3.9.6")
    parser.add_argument('--generate-nist-file', type=int, metavar='NUM_BITS', help="NIST file generation mode.")
    args = parser.parse_args()
    if args.generate_nist_file:
        asyncio.run(generate_nist_file(args.generate_nist_file))
    else:
        import uvicorn
        uvicorn.run("random_webcam_rng_v3.9.6:app", host="0.0.0.0", port=8000)